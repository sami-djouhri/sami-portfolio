...
export const PROJECT_DETAILS_EN_1: Record<string, ProjectDetail> = {
  saganta: {
    ...
    decisions: [
      {
        title: 'An auth service of its own, not a foreign OIDC stack',
    ...
    timeline: [
      { when: 'Q1 2026', what: 'Shell, own auth service and first sub-apps live' },
    ...
    metrics: [
      ...
      { label: 'Auth backend', value: 'own auth service' },
    ],
  },
  ...
  homelab: {
    ...
    approach: [
      ...
      'An event spine of its own: several producers (search, alerts, deals, mail, mailbox) write into one central MQTT bus, and brain-bus rules triage the alerts with dedup and cooldown.',
    ],
    architecture: {
      summary:
      ...
      tiers: [
        ...
        },
        {
          ...
          nodes: [
            ...
            { id: 'backup', label: 'Off-site backup', note: 'encrypted, DR runbook', kind: 'data' },
    ...
    decisions: [
      {
        ...
        body: 'Linkerd or Istio are overkill for a home setup. A reverse proxy, tunnels, VPN and segmented compose networks do exactly what is needed.',
      },
      {
    ...
    metrics: [
      { label: 'Live services', value: `${DIENSTE_PROSA.en.gross} across the fleet` },
      { label: 'Restore', value: 'drilled quarterly, compared byte for byte' },
  ...
  lernen: {
    ...
    approach: [
      ...
      'Shared web building blocks through a design system (`design-system`); Tailwind and Compose tokens are synced into every app, never copy-pasted.',
  ...
  'wissens-foederation': {
    ...
    result: [
      'Seven real sources are searchable through a single search API, every backend access token-secured.',
    ...
    decisions: [
      ...
      {
        title: 'Knowledge is not operational state',
        body: 'Four of the original eleven sources went back out: inventory, market data, appointments and postal mail. They were trivial to wire in, and that was exactly the trap. A knowledge search answers "what do I know about this", an application answers "what is the current state". Mixing the two yields hits that look stale while the application has long moved on, and you start synchronising freshness. Fewer sources, but one clear owner per question.',
      },
    ],
    metrics: [
      {
        label: 'Wired-in sources',
        value: '7, heterogeneous',
      },
      {
        label: 'Aggregation',
    ...
    timeline: [
      ...
      {
        when: '2026 H2',
        what: 'Four sources taken back out: a line between knowledge and operational state',
      },
    ],
  },
  ...
  'ai-vision': {
    ...
    result: [
      'A recognition model on my own edge hardware, wired into the rest of my stack, with warm inference in a fraction of a second.',
    ...
    decisions: [
      {
        ...
        body: 'A hardware upgrade is often simply not an option on the first generation. Running at it gets you nowhere, so there is a documented path that accepts the platform\'s quirks and gets productive precisely through that. The stumbling blocks become recorded knowledge, not recurring pain.',
      },
      {
    ...
    limits: [
      'Recognition depends on exactly one machine: if it goes down, recognition stops. During the re-cabling that was the case for a while. The rest of the stack does not notice, because image jobs stay queued instead of failing, which is exactly what the queue is for.',
      'The hardware is first-generation: tight memory, an old toolchain. That noticeably caps model size and turns every version bump into a small project of its own.',
    ],
  },
};
