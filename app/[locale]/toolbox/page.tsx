...
import { StackMark } from '../../components/DomainMark';
...
const RATIONALES_DE: Rationale[] = [
  {
    label: 'Infra & Ops',
    body: 'Docker Compose ist absichtlich kein Kubernetes. Ein Cluster-Scheduler löst hier kein Problem, das ich tatsächlich habe; lesbare Compose-Files mit Härtung pro Service schon. cloudflared statt offener Ports, ein VPN für persönliche Remote-Sessions, ein zentraler SSO-/OIDC-Anbieter davor.',
...
const RATIONALES_EN: Rationale[] = [
  {
    label: 'Infra & Ops',
    body: 'Docker Compose is deliberately not Kubernetes. A cluster scheduler solves no problem I actually have; readable compose files with per-service hardening do. cloudflared instead of open ports, a VPN for personal remote sessions, a central SSO/OIDC provider in front.',
...
const USES_DE: UsesEntry[] = [
  {
    ...
    items: [
      ...
      // ★ Bewusst OHNE Modellbezeichnungen (2026-09-15). Router und Gateway sind die
      // Netzgrenze zum Internet; ein genanntes Modell ist zusammen mit dem angegebenen
      // Raum und den verlinkten Domains eine fertige Vorlage für den Abgleich mit
      // bekannten Firmware-Lücken. Die fachliche Aussage (eigener Edge-Router, eigene
      // Zonen, Glasfaser) steht unverändert, nur Hersteller und Typ fehlen.
      { name: 'Eigener Edge-Router im Standalone-Mode, getrennte Netz-Zonen' },
      { name: 'Glasfaser-Anschluss, Anbieter-Router davor' },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'VPN', note: 'für persönliche Remote-Sessions' },
      { name: 'SSO / OIDC', note: 'zentrale Anmeldung für Eigen-Apps' },
  ...
  },
  {
    ...
    items: [
      ...
      // Anbieter bewusst ungenannt: wo die Sicherungen liegen, ist das zweite Ziel
      // eines Angreifers und trägt fachlich nichts bei.
      { name: 'Off-Site-Backup', note: 'verschlüsselt, bei einem EU-Anbieter' },
...
const USES_EN: UsesEntry[] = [
  {
    ...
    items: [
      ...
      // Siehe Kommentar an der deutschen Fassung: keine Modellnamen an der Netzgrenze.
      { name: 'Dedicated edge router in standalone mode, separated network zones' },
      { name: 'Fibre line, carrier router in front' },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'VPN', note: 'for personal remote sessions' },
      { name: 'SSO / OIDC', note: 'central sign-in for my own apps' },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Off-site backup', note: 'encrypted, with an EU provider' },
...
export default async function ToolboxPage(props: { params: Promise<{ locale: string }> }) {
  ...
  // Fuer die Shell: dieselben Daten, die diese Seite ohnehin rendert. Es geht
  // nichts an den Browser, was nicht schon im HTML steht.
  const shellDaten = {
    services: proof.services,
    hosts: proof.hosts,
    drift: proof.drift,
    measuredAgeHours: proof.measured?.age_hours ?? null,
    projekte: projects.map((p) => ({
      id: p.id,
      title: p.title,
      domain: p.domain,
      status: p.status,
      tagline: localizedProject(p, locale).tagline,
    })),
  };
  ...
  return (
    <div className="relative">
      ...
      <main id="main" className="mx-auto max-w-5xl px-6 pb-32 pt-12 sm:px-8 sm:pt-16">
        ...
        <HeaderVisual visual={<LayerStack className="ml-auto h-full w-auto opacity-50" />}>
          <PageHeader
            eyebrow="ls ~/toolbox"
        ...
        <section id="philosophie" className="scroll-mt-24 pt-20">
          ...
          {/* Disziplinen als Karten mit Zeichen und Werkzeug-Chips.
              ...
              ein Drittel der Höhe, ohne einen einzigen Namen wegzulassen. */}
          <Reveal as="div" stagger className="mt-8 grid gap-4 sm:grid-cols-2">
            {STACK.map((cat) => (
              <div
                key={cat.key}
                className="group overflow-hidden rounded-lg border border-border bg-surface/40 shadow-panel"
              >
                <div className="flex items-center gap-3 border-b border-border px-5 py-4">
                  <StackMark stack={cat.key} />
                  <h3 className="font-mono text-sm uppercase tracking-widest text-text">
                    {cat.label}
                  </h3>
                  <span className="nums ml-auto font-mono text-xs text-muted-dim">
                    {cat.items.length}
                  </span>
                </div>
                <ul className="flex flex-wrap gap-1.5 p-5">
                  {cat.items.map((it) => (
                    <li
                      key={it}
                      className="rounded-md border border-border bg-bg px-2 py-1 font-mono text-[0.7rem] text-text/85"
                    >
                      {it}
                    </li>
                  ))}
                ...
                return (
                  ...
                  >
                    {/* ★ Die Werkzeugliste stand hier ein ZWEITES Mal, vollständig,
                        ...
                        hier nur noch, WELCHE Disziplin begründet wird; die Begründung
                        selbst bekommt den Platz. */}
                    <div className="md:col-span-3">
                      <div className="flex items-center gap-3">
                        {cat ? <StackMark stack={cat.key} size="sm" /> : null}
                        <p className="font-mono text-xs uppercase tracking-widest text-term/85">
                          <span className="nums">{(i + 1).toString().padStart(2, '0')}</span> ·{' '}
                          {r.label}
                        </p>
                      </div>
                    </div>
        ...
        <section id="zahlen" className="scroll-mt-24 pt-20">
          ...
          >
            ...
            <div className="grid gap-px bg-border sm:grid-cols-2 md:grid-cols-12">
              {/* Dieselbe Aussage wie auf der Startseite bekommt dieselbe Bildsprache:
                  erst die Matrix (eine Zelle je Dienst), dann die Zahl als ihre
                  Beschriftung. Zwei verschiedene Darstellungen desselben Fakts auf
                  einer Website lassen sie wie zwei verschiedene Fakten aussehen. */}
              <div className="bg-bg p-6 sm:col-span-2 sm:p-8 md:col-span-6">
                <p className="label">{en ? 'In production' : 'Im Dauerbetrieb'}</p>
                <div className="mt-4">
                  <ServiceMatrix services={proof.services} drift={proof.drift ?? 0} id="stats" />
                </div>
                <div className="mt-4 flex items-baseline gap-3">
                  ...
                  <span className="text-sm leading-snug text-muted">
                    ...
                    {proof.drift !== null
                      ? en
                        ? ` · ${proof.drift} deviations from the map`
                        : ` · ${proof.drift} Abweichungen vom Soll`
                      : ''}
                  </span>
                </div>
              </div>
          ...
          <div className="pt-10">
            ...
            <TerminalShell locale={locale} daten={shellDaten} />
          </div>
          ...
          <div className="pt-16">
            ...
            <Reveal className="mt-8 overflow-hidden rounded-panel border border-border bg-surface/40 shadow-panel">
              ...
              <ol className="space-y-3 p-5 sm:p-7">
                {domainSorted.map(([domain, count]) => (
                  <li key={domain} className="bar-row">
          ...
          <div className="pt-16">
            ...
            <p className="mt-4 font-mono text-xs text-muted-dim">
              <span aria-hidden className="mr-2 text-term/80">›</span>
              {en
      ...
      <div className="p-5 sm:p-6">
        ...
        <ul className="mt-4 space-y-2.5 font-mono text-sm">
          {group.items.map((item) => (
            /* ★ Zwei feste Spalten statt `flex-wrap`: der Marker war ein eigenes
               Flex-Element und blieb auf 390 px allein auf seiner Zeile, waehrend der
               Text darunter am linken Rand begann, acht Zeilen lang wie leere
               Aufzaehlungspunkte. Der Trennpunkt haengt jetzt ueber ein geschuetztes
               Leerzeichen am Namen, statt als vierter, kaputt wirkender Eintrag eine
               Zeile zu beginnen. */
            <li key={item.name} className="grid grid-cols-[auto_1fr] items-baseline gap-x-2">
