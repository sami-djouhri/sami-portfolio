...
export const CV_EXPERIENCE: CvExperience[] = [
  {
    ...
    role: 'Homelab-Infrastruktur & Eigen-Systeme · Eigenbetrieb',
    ...
    bullets: [
      ...
      'Eigen-Productivity-Suite (Saganta), SvelteKit-Shells vor FastAPI-BFFs, eigener schlanker Auth-Dienst (better-auth) statt Fremd-OIDC, eine Session über Web und native App, eigener Event-Spine für Cross-Service-Signale.',
  ...
  {
    period: '2024 bis heute',
    role: 'Eigen-Projekte · Web-Hosting & WordPress-Härtung',
    ...
    bullets: [
      ...
      'Reverse-Proxy mit automatischem TLS pro Instanz (Caddy · cloudflared), sauber getrennte Vhosts statt geteiltem Zugang.',
      'Getrennte Datenbank- und Datei-Backups je Instanz mit dokumentiertem Restore-Pfad, statt Wiederherstellung nur zu hoffen.',
...
export const CV_HIGHLIGHTS: string[] = [
  ...
  'Virtualisierung im Eigenbetrieb: drei-Knoten-Proxmox-Cluster mit Containern und VMs, Windows-Server- und Active-Directory-Lab, schwere Dienste bedarfsgesteuert statt dauerhaft belegt.',
  'Monitoring, Alerting und verschlüsseltes Off-Site-Backup mit getestetem Restore im Regelbetrieb, statt Wiederherstellung nur zu hoffen.',
  'Container-Betrieb mit Docker Compose, segmentierten Netzen und VPN-Zugang, gehärtet und pro Ausnahme dokumentiert.',
  'Architektur schreibend dokumentiert: Zielbild, Runbooks und Service-Map als Pflicht, damit Betrieb reproduzierbar und übergebbar bleibt, statt im Kopf einer Person zu hängen.',
];
...
const CV_EXPERIENCE_EN: CvExperience[] = [
  {
    ...
    bullets: [
      ...
      'Own productivity suite (Saganta): SvelteKit shells in front of FastAPI BFFs, a self-built lean auth service (better-auth) instead of third-party OIDC, one session across web and native app, a custom event spine for cross-service signals.',
  ...
  },
  {
    ...
    bullets: [
      ...
      'Reverse proxy with automatic TLS per instance (Caddy · cloudflared), cleanly separated vhosts instead of shared access.',
      'Separate database and file backups per instance with a documented restore path, instead of merely hoping recovery works.',
...
const CV_HIGHLIGHTS_EN: string[] = [
  ...
  'Self-run virtualization: a three-node Proxmox cluster with containers and VMs, a Windows Server and Active Directory lab, and heavy services run on demand instead of permanently resident.',
  'Monitoring, alerting and encrypted off-site backup with a tested restore as routine, instead of merely hoping recovery works.',
  'Container operations with Docker Compose, segmented networks and VPN access, hardened and documented per exception.',
  'Architecture documented as I go: target picture, runbooks and service map are mandatory, so operations stay reproducible and easy to hand over instead of living in one person’s head.',
];
