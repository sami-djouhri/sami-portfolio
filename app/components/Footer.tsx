...
import { CV_OEFFENTLICH, PORTFOLIO_REPO_URL, SOCIAL_LINKS } from '@/lib/site';
...
export function Footer({ locale }: { locale: Locale }) {
  ...
  const resources: FooterLink[] = [
    // Lebenslauf und PDF hängen am Schalter CV_OEFFENTLICH (lib/cv.ts, seit
    // 2026-08-31 vorübergehend aus). Die Routen antworten selbst mit 404; hier
    // verschwindet der Verweis, damit die Fußzeile nicht auf eine 404 zeigt.
    ...(CV_OEFFENTLICH
      ? [
          { href: localePath(locale, '/cv'), label: t(locale, 'nav.cv') },
          {
            href: `/cv.pdf?lang=${locale}`,
