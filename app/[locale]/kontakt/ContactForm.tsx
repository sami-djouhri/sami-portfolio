...
export function ContactForm({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="mt-10 overflow-hidden rounded-lg border border-border bg-surface/40">
      ...
      >
        <Field label={t(locale, 'form.name')} name="name" autoComplete="name" required />
        <Field label={t(locale, 'form.email')} name="email" type="email" autoComplete="email" required />
        <Field
          label={`${t(locale, 'form.subject')} (optional)`}
          name="subject"
          autoComplete="off"
        />
        <TextArea label={t(locale, 'form.message')} name="message" required />
        ...
        <div className="flex flex-wrap items-center gap-4">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-md bg-accent px-4 py-2 text-sm font-medium text-bg transition-all hover:bg-accent-bright hover:shadow-glow disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-accent disabled:hover:shadow-none"
          >
            {state === 'sending' ? (
              <>
                {en ? 'sending' : 'sende'}
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="mt-2 block w-full rounded-md border border-border bg-elev px-3 py-2 font-mono text-sm text-text shadow-[inset_0_1px_2px_rgba(0,0,0,0.35)] transition-colors placeholder:text-muted-dim focus:border-accent focus:outline-none"
      />
    </label>
  );
}
  ...
  return (
    <label className="block">
      ...
      <textarea
        ...
        rows={6}
        aria-required={required || undefined}
        className="mt-2 block w-full resize-y rounded-md border border-border bg-elev px-3 py-2 font-mono text-sm leading-relaxed text-text shadow-[inset_0_1px_2px_rgba(0,0,0,0.35)] transition-colors placeholder:text-muted-dim focus:border-accent focus:outline-none"
      />
    </label>
  );
}
