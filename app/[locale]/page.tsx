...
const HOME_DESC: Record<Locale, { description: string; og: string }> = {
  de: {
    description:
      'Sami Djouhri baut und betreibt eigene Systeme: ein gehärtetes Homelab mit über 160 Diensten, dazu eigene Suiten und lokale AI, wo sie trägt.',
    og: 'Über 160 selbst betriebene Dienste, gehärtet und dokumentiert. Infrastruktur, Betrieb und Automation, selbst gebaut.',
  },
  en: {
    description:
      'Sami Djouhri builds and runs his own systems: a hardened homelab backbone, operations and automation, plus his own suites and local AI where it earns its place. All of it self-built and self-run.',
    og: 'Over 160 self-run services, visible hardening and documented care. Infrastructure, operations and automation, self-built.',
  },
};
  ...
  return (
    <section className="pb-16 pt-4 sm:pt-8">
      ...
      <div className="mt-10 grid gap-10 md:grid-cols-12">
        ...
        <aside className="relative isolate md:col-span-5">
          <div
            aria-hidden
      ...
      {live.reachable ? (
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-border bg-bg/40 px-4 py-2.5 font-mono text-xs">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="group ml-auto inline-flex items-center gap-1 py-1 text-muted-dim transition-colors hover:text-accent"
          >
...
function SectionHead({ command, title, index }: { command: string; title: string; index: string }) {
  return (
    <div className="flex items-end justify-between gap-6 border-b border-border pb-4">
      ...
      <span aria-hidden className="hidden font-mono text-sm text-accent/70 sm:block">[{index}]</span>
    </div>
  );
}
