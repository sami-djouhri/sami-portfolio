...
import { sendContactAck, sendContactMail, smtpConfigured } from '@/lib/contact-mail';
...
interface ContactPayload {
  ...
  /** Sprache des Formulars, entscheidet die Sprache der Empfangsbestätigung. */
  locale?: unknown;
}
...
export async function POST(request: Request) {
  ...
  // CMS-Posteingang (immer) oder optionaler SMTP-Versand (Mailserver im DMZ-Netz). Erst wenn
  ...
  // 3) Empfangsbestätigung an den Absender. Bewusst NACH der captured-Prüfung und
  //    bewusst ohne Einfluss auf den Statuscode: bestätigt wird nur, was auch
  //    wirklich erfasst ist, und eine gescheiterte Bestätigung darf keine
  //    Fehlermeldung erzeugen, obwohl die Nachricht angekommen ist.
  await sendContactAck(email, asLocale(typeof body.locale === 'string' ? body.locale : null));
