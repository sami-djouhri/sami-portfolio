  ...
  return (
    <footer role="contentinfo" className="mx-auto max-w-5xl px-6 pb-12 pt-16 sm:px-8 no-print">
      ...
      <div className="mt-12 grid gap-10 sm:grid-cols-12">
        ...
        <FooterCol heading={t(locale, 'footer.navigation')} links={navigation} className="sm:col-span-3" />
        <FooterCol heading={t(locale, 'footer.resources')} links={resources} className="sm:col-span-2" />
        <FooterCol heading={t(locale, 'footer.legal')} links={legal} className="sm:col-span-2" />
      </div>
            ...
            {SOCIAL_LINKS.map((s) =>
              s.href ? (
                ...
                <span
                  ...
                  className="inline-flex h-9 cursor-default items-center gap-2 rounded-md border border-border/50 px-3 font-mono text-xs text-muted-dim/45"
                >
                  <SocialIcon label={s.label} size={15} />
                  <span>{s.label}</span>
                </span>
