...
export function StatusBadge({
  ...
  suffix,
  /**
   * In langen Listen den Normalfall zurücknehmen.
   *
   * In einer Sektion, die ohnehin „Im Betrieb“ heißt, wiederholt sich „live“ zwanzigmal
   * untereinander. Zwanzig identische grüne Punkte sind keine Statusmeldung mehr, sondern
   * Tapete, und sie kosten `term`-Grün genau die Signalwirkung, für die es reserviert ist
   * (Doktrin: funktional, nie dekorativ). Mit `quietWhen` bleibt beim Normalfall nur die
   * Jahresangabe stehen, sodass jede Abweichung davon sofort ins Auge fällt.
   */
  quietWhen,
}: {
  ...
  locale: Locale;
  suffix?: string;
  quietWhen?: ProjectStatus;
}) {
  if (quietWhen && status === quietWhen && suffix) {
    return (
