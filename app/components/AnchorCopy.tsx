...
export function AnchorCopy({ id, label }: { id: string; label: string }) {
  ...
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className="ml-3 inline-block align-middle font-mono text-2xl text-muted-dim opacity-0 transition-opacity duration-[140ms] hover:text-accent focus-visible:opacity-100 group-hover:opacity-100"
    >
      {copied ? '✓' : '#'}
    </button>
  );
