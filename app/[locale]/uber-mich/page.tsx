  ...
  return (
    <div className="relative">
      ...
      <main id="main" className="mx-auto max-w-3xl px-6 pb-32 pt-12 sm:px-8 sm:pt-16">
        ...
        {/* Reihenfolge ist Absicht und wurde am 2026-08-27 gedreht: vorher stand an
            Position 2 ein reines Inventar (acht Zeilen Aufzaehlung), und die einzige
            echte Geschichte der Seite — aus einer Platine wird ein Verbund — kam erst
            danach. Jetzt: wer ich bin, wie es anfing, was es mich gelehrt hat, was
            ...
            Die zwei Lehren standen bis dahin NUR in der englischen Fassung. Die
            Hauptsprache hatte damit die schwaechere Erzaehlung. Sie sind hier bewusst
            kurz gehalten; die ausfuehrlichen Fassungen liegen unveroeffentlicht in
            lib/notes.ts (Owner-Review offen) und werden von hier NICHT verlinkt. */}
        <div className="prose-editorial mt-12">
          ...
          <p>
            {en ? (
              <>
                Two things taught me more than any tutorial. First: I had secured the network so
                thoroughly that only my work laptop could still get in, until it hit the floor and
                died. The keys lived on it, and it had no backup, because &bdquo;the work is on the
                servers anyway&ldquo;. Every hardening step gets a planned way back in now. Second:
                the suite was built for exactly one person. Retrofitting a second account cost more
                than any feature before it, because the assumption &bdquo;there is only one&ldquo;
                sat in every seam. New services start out multi-user here, even when nobody but me
                uses them yet.
              </>
            ) : (
              <>
                Zwei Sachen haben mich mehr gelehrt als jedes Tutorial. Erstens: Ich hatte das Netz
                so weit abgesichert, dass nur noch mein Arbeitslaptop hineinkam, bis der runterfiel
                und kaputt war. Auf ihm lagen die Schlüssel, und gesichert war er nicht, weil ja
                &bdquo;die Arbeit auf den Servern liegt&ldquo;. Seitdem bekommt jede Absicherung
                vorher einen geplanten Rückweg. Zweitens: Die Suite war für genau einen Menschen
                gebaut. Ein zweites Konto nachzurüsten kostete mehr als jede Funktion davor, weil
                die Annahme &bdquo;es gibt nur einen&ldquo; in jeder Naht steckte. Neue Dienste
                starten hier seitdem mehrbenutzerfähig, auch wenn sie vorerst niemand außer mir
                benutzt.
              </>
            )}
          </p>
