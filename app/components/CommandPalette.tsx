    ...
    >
      ...
      >
        <div className="flex items-center gap-3 border-b border-border bg-bg/60 px-4 py-2">
          ...
          <span className="ml-auto font-mono text-[0.6rem] uppercase tracking-widest text-muted-dim">
            esc
          </span>
        </div>
            ...
            filtered.map((c, i) => {
              ...
              return (
                <li key={c.id} id={`cmdk-item-${i}`} role="option" aria-selected={i === active} data-idx={i}>
                  {showHeader ? (
                    <p className="px-4 pb-1 pt-2 font-mono text-[0.6rem] uppercase tracking-widest text-muted-dim">
                      {t(locale, c.group)}
                    </p>
                  ) : null}
                  <button
