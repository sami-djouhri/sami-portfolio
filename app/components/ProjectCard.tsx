    ...
    <li className={`group ${quiet ? 'opacity-80' : ''}`}>
      ...
      >
        ...
        <div className="flex min-w-0 items-baseline gap-2.5 sm:w-64 sm:shrink-0">
          ...
          <span className="hidden shrink-0 font-mono text-[0.6rem] tracking-widest text-muted-dim sm:inline">
            ~/{project.id}
          </span>
        </div>
        ...
        {matched.length > 0 ? (
          <div className="flex shrink-0 flex-wrap items-center gap-1">
            {matched.map((s) => (
              <span
                key={s}
                className="rounded border border-accent/30 bg-accent/10 px-1.5 py-0.5 font-mono text-[0.6rem] text-accent"
              >
                {s}
              </span>
            ))}
...
export function ProjectCardCompact({
  ...
  return (
    ...
    >
      ...
      {hasPreview ? (
        <PreviewThumb id={project.id} />
      ) : featured ? (
        <CardVisual id={project.id} domain={project.domain} />
      ) : null}
