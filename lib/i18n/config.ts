 ...
 *
 * URL-Modell: Pfad pro Sprache (/de, /en). DE ist Default. Die proxy.ts leitet
 ...
 * app/[locale]/ bleiben deutsch: proxy.ts rewritet /en/<en-slug> intern auf
...
/**
 * Metadata-`openGraph` für eine Seite, die eine eigene OG-Beschreibung braucht.
 *
 * ★ Nötig, weil `openGraph` zwischen Segmenten NICHT gemischt, sondern vom letzten
 * Segment ERSETZT wird (Next-Doku, „Merging“: verschachtelte Felder eines früheren
 * Segments werden überschrieben). Eine Seite, die nur `openGraph: { description }`
 * setzt, löscht damit still Bild, Typ, Sprache und Seitennamen aus dem Layout, und
 * beim Teilen erscheint eine nackte Textzeile ohne Vorschaubild. Genau das war auf
 * /toolbox und /jetzt der Fall.
 *
 * Wer nur den Titel braucht, setzt hier NICHTS: ohne eigenes `openGraph` erbt die
 * Seite das Layout-Objekt, und Next leitet `og:title` aus ihrem `title` ab.
 */
export function localeOpenGraph(
  ...
  return {
    description,
    url: localeAlternates(locale, path).canonical,
    siteName,
    locale: locale === 'en' ? 'en_US' : 'de_DE',
    alternateLocale: locale === 'en' ? 'de_DE' : 'en_US',
    type: 'website' as const,
    images: [`/${locale}/opengraph-image/main`],
  };
}
