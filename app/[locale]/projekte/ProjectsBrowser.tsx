...
/**
 * Der Status, der in einer Gruppe der Normalfall ist, und deshalb zurückgenommen wird.
 *
 * Zwanzig identische „live“-Marker untereinander sind keine Statusmeldung mehr, sondern
 * Tapete, und sie kosten dem funktionalen Grün seine Signalwirkung. Beruhigt wird nur,
 * wenn ein Status echte Mehrheit hat und die Gruppe groß genug ist, damit die Anzeige
 * bei zwei Einträgen nicht willkürlich wirkt. Abweichungen behalten Punkt und Label und
 * fallen dadurch von selbst auf.
 */
function groupQuietStatus(items: Project[]): ProjectStatus | undefined {
  if (items.length < 3) return undefined;
  const counts = new Map<ProjectStatus, number>();
  for (const p of items) counts.set(p.status, (counts.get(p.status) ?? 0) + 1);
  let best: ProjectStatus | undefined;
  let bestN = 0;
  for (const [s, n] of counts) if (n > bestN) [best, bestN] = [s, n];
  return bestN > items.length / 2 ? best : undefined;
}
...
export function ProjectsBrowser({
  ...
  const [alleStacks, setAlleStacks] = useState(false);
  ...
  /**
   * Sichtbare Stack-Chips. Von den 63 Einträgen kommen rund vierzig genau einmal
   ...
   */
  const SCHWELLE = 2;
  const MIN_SICHTBAR = 12;
  const sichtbareStacks = useMemo(() => {
    if (alleStacks) return allStacks;
    const mehrfach = allStacks.filter(
      ([stack, count]) => count >= SCHWELLE || activeStacks.has(stack),
    );
    ...
    const basis =
      mehrfach.length >= MIN_SICHTBAR ? mehrfach : allStacks.slice(0, MIN_SICHTBAR);
    // Ein per URL gesetzter Filter darf nie unsichtbar werden.
    const fehlendeAktive = allStacks.filter(
      ([stack]) => activeStacks.has(stack) && !basis.some(([b]) => b === stack),
    );
    return [...basis, ...fehlendeAktive];
  }, [allStacks, alleStacks, activeStacks]);
  const versteckteStacks = allStacks.length - sichtbareStacks.length;
    ...
    <>
      ...
      >
        ...
        <div className="p-5">
          <div className="flex items-baseline justify-between gap-4 font-mono text-xs">
            ...
            {hasFilter ? (
              <button
                type="button"
                onClick={reset}
                className="-my-1 px-1.5 py-1 uppercase tracking-widest text-muted hover:text-accent"
              >
                reset · {hitsLabel}
              </button>
            ) : (
          ...
          <div className="mt-4 flex flex-wrap gap-1.5">
            {sichtbareStacks.map(([stack, count]) => {
              ...
              return (
                <button
                  ...
                  className={
                    active
                      ? 'min-h-6 rounded-md border border-accent bg-accent/15 px-2 py-1 font-mono text-[0.7rem] text-accent transition-colors'
                      : 'min-h-6 rounded-md border border-border bg-elev px-2 py-1 font-mono text-[0.7rem] text-muted transition-colors hover:border-border-strong hover:text-text'
                  }
                >
                  {stack}
                  <span className="ml-1.5 text-muted-dim">{count}</span>
            ...
            {versteckteStacks > 0 || alleStacks ? (
              <button
                type="button"
                onClick={() => setAlleStacks((v) => !v)}
                aria-expanded={alleStacks}
                className="min-h-6 rounded-md border border-dashed border-border px-2 py-1 font-mono text-[0.7rem] text-muted-dim transition-colors hover:border-accent/60 hover:text-accent"
              >
                {alleStacks
                  ? locale === 'en'
                    ? 'show fewer'
                    : 'weniger zeigen'
                  : locale === 'en'
                    ? `+${versteckteStacks} used once`
                    : `+${versteckteStacks} einmal genutzt`}
              </button>
            ) : null}
          </div>
        </div>
            ...
            <Reveal as="ul" stagger className="divide-y divide-border/50 px-5">
              {items.map((p) => (
                <ProjectListItem
                  ...
                  quietStatus={groupQuietStatus(items)}
                />
              ))}
            </Reveal>
          </section>
