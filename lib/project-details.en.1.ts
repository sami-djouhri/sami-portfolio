...
export const PROJECT_DETAILS_EN_1: Record<string, ProjectDetail> = {
  ...
  'ai-vision': {
    ...
    limits: [
      'The edge hardware is currently off the network and the pipeline is waiting for it. Reactivation from a different storage device is prepared but not done. The rest of the stack does not notice, because image jobs stay queued instead of failing, which is exactly what the queue is for.',
