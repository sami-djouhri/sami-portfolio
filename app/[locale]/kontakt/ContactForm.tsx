...
export function ContactForm({ locale }: { locale: Locale }) {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      captchaToken,
      // Nur fuer die Sprache der Empfangsbestaetigung. Serverseitig durch
      // asLocale() gefiltert, ein manipulierter Wert faellt auf 'de' zurueck.
      locale,
    };
    ...
    return (
      <div className="mt-10 overflow-hidden rounded-lg border border-term/40 bg-surface/40">
        ...
        >
          ...
          {/* Das „was jetzt" nach dem Absenden. Zeigt auf den Lebenslauf, solange
              er öffentlich ist (CV_OEFFENTLICH, lib/site.ts), sonst auf die
              Projekte. Der Absatz bleibt in jedem Fall stehen: eine
              Bestätigungsseite ohne Weiterweg ist eine Sackgasse. */}
          <p className="mt-4 font-mono text-sm text-muted">
            <span aria-hidden className="text-term">›</span>{' '}
            <Link
              href={localePath(locale, CV_OEFFENTLICH ? '/cv' : '/projekte')}
              className="text-text transition-colors hover:text-accent"
            >
              {CV_OEFFENTLICH ? '/cv' : '/projekte'}
            </Link>{' '}
            <span className="text-muted-dim">
              ,{' '}
              {CV_OEFFENTLICH
                ? en
                  ? 'view the résumé in the meantime'
                  : 'in der Zwischenzeit den Lebenslauf ansehen'
                : en
                  ? 'browse the projects in the meantime'
                  : 'in der Zwischenzeit die Projekte ansehen'}
            </span>
          </p>
        </div>
      </div>
