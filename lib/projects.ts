...
import { DIENSTE_PROSA } from './proof';
 ...
 * niemals Hostnames, IPs oder Container-Namen (Privacy-Regel).
 */
 ...
 */
/**
 * `year` ist eine Phrase („seit 2026“), keine Zahl, und wurde deshalb von
 * `localizedProject` nie mit übersetzt: auf /en stand das deutsche „seit“ an jeder
 * der 25 Projektkarten und zusätzlich im Zeitraum-Feld jeder Fallstudie.
 */
export function localizedYear(year: string, locale: Locale): string {
  return locale === 'en' ? year.replace(/^seit /, 'since ') : year;
}
...
export const PROJECTS: Project[] = [
  ...
  },
  {
    id: 'homelab',
    title: 'Homelab',
    tagline: `${DIENSTE_PROSA.de.gross} Dienste auf mehreren Hosts, seit 2024 im Dauerbetrieb.`,
    // Die Beschreibung setzt die Tagline FORT, sie wiederholt sie nicht. Beide
    // begannen bis 2026-08-31 mit „Über 160 … im Dauerbetrieb", und auf der
    // Startseite rendern sie als Featured-Karte direkt untereinander: derselbe
    // Satz zweimal, zwei Zeilen auseinander.
    description:
    ...
    year: 'seit 2024',
    highlight: 'Jeder Dienst steht in einer versionierten Service-Map, gegen die der Ist-Zustand gemessen wird',
  ...
  },
  {
    ...
    role: 'Konzept, Aufbau, Raum-Modell, Betrieb',
    year: 'seit 2026',
  ...
  },
  {
    ...
    role: 'Konzept, Aufbau, Sicherheitsmodell',
    year: 'seit 2026',
  ...
  },
  {
    ...
    tagline: 'Ein Discord-Bot als Bereitschaftsdienst fürs Homelab.',
    description:
    ...
    role: 'Konzept, Aufbau, Event-Anbindung',
    year: 'seit 2026',
  ...
  },
  {
    ...
    role: 'Konzept, Datenpfad, Bewertungs-Regeln, Betrieb',
    year: 'seit 2025',
  ...
  },
  {
    ...
    tagline: 'Bilderkennung auf Edge-Hardware im Heimnetz.',
    description:
    ...
    year: 'seit 2026',
    highlight: 'Lokale GPU-Inferenz im Heimnetz, unabhängig von der Cloud. Nach dem Umbau der Verkabelung aus gesichertem Stand wieder angelaufen.',
  ...
  },
  {
    ...
    tagline: 'Eine Such-API über sieben Wissensquellen im Haus. Das Frontend kennt keine davon.',
    description: 'Ich wollte nicht jedes Mal überlegen, ob eine Information gerade in einer Notiz, einem Dokument oder einem Nachschlagewerk liegt. Deshalb fragt die Suche mehrere Systeme gleichzeitig ab und bringt die Treffer in ein gemeinsames Format. Die eigentlichen Daten bleiben in ihren jeweiligen Anwendungen. Sieben Quellen hängen dran: Dokumente, Notizen, Nachschlagewerke, Bücher, Fachwissen, Kursmaterial und meine eigenen Betriebsanleitungen. Es waren einmal elf, bis ich vier wieder ausgebaut habe: Inventar, Marktdaten, Termine und Post sind Anwendungsdaten, sie gehören in ihre Anwendung und nicht in einen Wissensindex. Jede Quelle steckt hinter einem eigenen Adapter, die Abfrage fächert sich auf alle auf. Aggregiert wird zur Abfragezeit, damit kein Hintergrund-Crawler eine zweite Kopie der Daten pflegen muss. Jeder Backend-Zugriff läuft token-authentifiziert.',
    role: 'Architektur, Adapter-Design, Betrieb',
    year: 'seit 2026',
    highlight: 'Sieben heterogene Quellen hinter einer Such-API, Auth pro Backend, Betriebsdaten bewusst draußen',
  ...
  },
  {
    ...
    role: 'Architektur, Verarbeitungskette, Kostensteuerung',
    year: 'seit 2026',
  ...
  },
  {
    ...
    tagline: 'Ein Sprach- und Text-Assistent, der jede Anfrage zum passenden Modell routet, lokal, wo es reicht.',
    ...
    role: 'Architektur, Routing-Regeln, Betrieb',
    year: 'seit 2026',
  ...
  },
  {
    ...
    role: 'Architektur, Härtung, Betrieb',
    year: 'seit 2026',
  ...
  },
  {
    ...
    tagline: 'Ein Lernlabor für Windows Server und Active Directory, gerade im Aufbau.',
    description:
  ...
  },
  {
    ...
    role: 'Architektur, Aufbau, Betrieb',
    year: 'seit 2026',
  ...
  },
  {
    id: 'rackforge',
    title: 'RackForge',
    tagline: 'Ein Shop für 3D-gedruckte Teile, ohne Baukasten-System.',
    description:
      'Ein 3D-Druck-Shop, der ohne Shop-Framework auskommt. Der Warenkorb behandelt dieselbe Form in zwei Farben als zwei Positionen, sonst stimmt die Kommissionierung nicht. Die Bestands-Engine führt Filament-Rollen und fertig gedruckte Teile getrennt: was nicht auf Lager liegt, läuft als „auf Bestellung“, ein erledigter Druck wird gegen den Bestand verbucht. Kunden laden eigene Druckdateien hoch, durch einen eigenen Upload-Pfad mit Format- und Größen-Grenzen. Datenmodell, Kasse und Bestands-Regeln gehören zum Eigenbau, nicht zu einer Plattform.',
    role: 'Konzept, Aufbau, Shop-Regeln, Betrieb',
    year: 'seit 2026',
  ...
  },
  {
    ...
    description:
      'Ein schwergewichtiges Fremd-CMS lohnt sich für eine kleine Seite selten, also liegt hier eine eigene Vorlage. Zwei Seiten laufen bereits damit und teilen sich dasselbe Abbild, eine davon öffentlich. Inhalte pflege ich über eine Oberfläche, die nur aus meinem eigenen Netz erreichbar ist; am öffentlichen Rand wird der Admin-Pfad abgewiesen, bevor die Anwendung ihn überhaupt sieht. Der öffentliche Teil wird statisch aus einem eigenen Datenspeicher ausgeliefert.',
    role: 'Konzept, Aufbau, Inhaltspflege, Betrieb',
    year: 'seit 2026',
    ...
    repos: [
      {
        ...
        note: 'Externer Wächter auf einem Off-Site-Server: Uptime-Monitoring, Status-Seite, Backup-Verifikation und ein Dead-Man-Switch fürs Heimnetz.',
        noteEn: 'External watchdog on an off-site VPS: uptime monitoring, status page, backup verification and a dead-man switch for the home lab.',
      },
    ],
  },
  {
    ...
    stack: ['Python', 'Dateisystem-Verschlüsselung', 'Docker Compose', 'Linux'],
    domain: 'Infra',
    status: 'im-aufbau',
  ...
  },
  {
    ...
    description:
      'Sieben schwere Rollen teilen sich einen Wirt, dessen Arbeitsspeicher bei weitem nicht für alle reicht. Überbuchen wäre der bequeme Weg, überlässt aber im Ernstfall dem Kernel die Entscheidung, welcher Prozess stirbt. Also plant ein eigener Regler den Speicher. Die Rollen stehen als Einträge in einer Registrierdatei, er fragt jede laufende Rolle nach ihrer tatsächlichen Auslastung, entscheidet über jeden Start am gemessenen freien Platz und schaltet ab, was zwanzig Minuten leer läuft. Erzwungen verdrängt wird nie: ist es zu eng, weicht zuerst, was leer und ungeschützt ist, sonst kommt eine Absage im Klartext. Anfangs hing daran eine zweite, aufwendige Hälfte, die nur die Anschluss-Adresse verbergen sollte, ein Tunnel und eine Weiterleitungsregel je Rolle. Mit dem Umzug der Rollen auf einen eigenen Server fiel sie ersatzlos weg. Dasselbe Prinzip trägt inzwischen auch die gewöhnlichen Web-Dienste: sieben selten benutzte Anwendungen auf zwei weiteren Rechnern starten beim ersten Aufruf und schlafen nach einer halben Stunde ohne Zugriff wieder ein.',
    role: 'Konzept, Netz-Design, Aufbau, Betrieb',
    year: 'seit 2026',
    highlight: 'Sieben schwere Rollen auf knappem Speicher, gestartet nach gemessenem Platz statt nach festem Slot, und wer gerade arbeitet, wird nicht verdrängt.',
    stack: ['Python', 'Proxmox · LXC', 'systemd', 'nginx', 'VPN', 'Paketfilter'],
    domain: 'Infra',
    status: 'live',
    repos: [
      {
        ...
        note: 'Regler für speicherknappe Wirte: entscheidet über jeden Start am gemessenen freien Platz, räumt belegte Rollen nie erzwungen, sichert den Zustand weichender Rollen und schaltet bei Leerlauf ab.',
  ...
  // ★ Aufgenommen am 2026-09-12. Der Bestand lief da schon ueber ein Jahr, stand
  // aber nirgends: das Portfolio kannte nur den Regler dahinter (ressourcen-arbiter),
  // nicht die Dienste, die er verwaltet. Fuer eine Bewerbung als Administrator ist
  // das der untypischste und zugleich staerkste Eintrag der Liste, weil hier zum
  // einzigen Mal FREMDE Menschen von der Verfuegbarkeit abhaengen und weil nichts
  // davon eigene Software ist. Reiner Betrieb, keine Zurechnungsfrage.
  {
    id: 'spielserver',
    title: 'Spielserver-Betrieb',
    tagline: 'Sieben öffentliche Server für eine feste Gruppe, im Wechsel statt gleichzeitig.',
    description:
      'Sieben Spielserver stehen offen im Netz und werden von einer festen Gruppe genutzt. Das ist der einzige Teil meiner Infrastruktur, dessen Ausfall andere Leute merken. Jeder Server läuft unter einem eigenen Dienstnutzer auf einem gemieteten Wirt, mit einer Firewall-Regel je Port, einer Ratenbegrenzung je Quelladresse noch vor der Firewall und Sperrlisten gegen Anmelde-Fluten. Der Speicher reicht nicht für alle gleichzeitig, also laufen sie auf Abruf: ein Platzhalter hält den schlafenden Server in der Serverliste sichtbar und weckt ihn beim Beitritt, nach zwanzig Minuten ohne Spieler geht er wieder schlafen. Wer gerade spielt, wird dabei nie verdrängt. Spielstände werden vor dem Herunterfahren gesichert und liegen doppelt off-site.',
    role: 'Aufbau, Härtung, Betrieb, Sicherung',
    year: 'seit 2026',
    highlight:
      'Sieben öffentlich erreichbare Server auf einem Wirt, dessen Speicher für zwei davon reicht',
    stack: ['Linux', 'systemd', 'Paketfilter', 'Ratenbegrenzung', 'Off-Site-Backup'],
    domain: 'Infra',
    status: 'live',
  },
];
...
> = {
  ...
  lernen: {
    ...
    role: 'Concept, build-out, operations',
  },
  'home-digital-twin': {
    ...
    role: 'Concept, build-out, room model, operations',
  },
  'homelab-app': {
    ...
    role: 'Concept, build-out, security model',
  },
  'homelab-sentinel': {
    ...
    role: 'Concept, build-out, event wiring',
  },
  marktwatch: {
    ...
    role: 'Concept, data path, scoring rules, operations',
  },
  'ai-vision': {
    tagline: 'Image recognition on edge hardware, inside the home network.',
    description:
    ...
    highlight: 'Local GPU inference inside the home network, independent of any cloud. Back up and running from a secured state after the re-cabling.',
  ...
  'wissens-foederation': {
    tagline: 'One search API over seven knowledge sources of my own. The frontend knows none of them.',
    description:
      'A single query searches seven completely different self-run systems at once (documents, notes, reference archives, books, domain knowledge, course material and my own runbooks), and the frontend knows none of them. There used to be eleven, until I took four back out: inventory, market data, appointments and postal mail are application data, they belong in their application rather than in a knowledge index. Each source sits behind an adapter, the query fans out to all of them and comes back unified; aggregation happens deliberately at query time, so no crawler has to maintain a second source of truth. Every backend access is token-authenticated. Scattered island systems become one knowledge layer that other services address as a single backend, without knowing the sources behind it.',
    highlight: 'Seven heterogeneous sources behind one search API, auth per backend, operational data deliberately left out',
  ...
  'news-engine': {
    ...
    role: 'Architecture, processing chain, cost control',
  },
  concierge: {
    ...
    role: 'Architecture, routing rules, operations',
  },
  'defense-in-depth': {
  ...
  'ops-cockpit': {
    ...
    role: 'Architecture, hardening, operations',
  },
  gartiko: {
  ...
  'ressourcen-arbiter': {
    ...
    description:
      'Seven heavy roles share a host whose memory is nowhere near enough for all of them. Overcommitting would be the comfortable route, but it hands the kernel the decision about which process dies. So a controller of my own schedules the memory. The roles live as entries in a registry file, it asks every running role for its actual occupancy, decides each start against measured headroom, and switches off whatever has been idle for twenty minutes. Nothing is ever evicted by force: when it gets tight, what is empty and unprotected gives way first, otherwise the answer is a plain refusal. Early on there was a second, elaborate half whose only job was to hide the home address, a tunnel and a forwarding rule per role. When the roles moved to a dedicated server, it fell away with nothing put in its place. The same principle now carries the ordinary web services too: seven rarely used applications on two further machines start on the first request and go back to sleep after half an hour without access.',
    highlight: 'Seven heavy roles on tight memory, started against measured headroom rather than a fixed slot, and whoever is working does not get evicted.',
    role: 'Concept, network design, build-out, operations',
  },
  'modell-vermittlung': {
    ...
    role: 'Architecture, build-out, operations',
  },
  spielserver: {
    tagline: 'Seven public servers for a fixed group, in rotation rather than all at once.',
    description:
      'Seven game servers sit openly on the net and are used by a fixed group. This is the one part of my infrastructure whose downtime other people notice. Each server runs under its own service user on a rented host, with one firewall rule per port, a per-source rate limit that bites before the firewall does, and ban lists against login floods. Memory is not enough for all of them at once, so they run on demand: a stand-in keeps the sleeping server visible in the server list and wakes it on join, and after twenty minutes without players it goes back to sleep. Nobody currently playing is ever evicted for it. Saves are written before shutdown and kept off-site twice over.',
    highlight: 'Seven publicly reachable servers on one host with memory for two of them',
    role: 'Build-out, hardening, operations, backups',
  },
};
...
export const ABOUT = {
  ...
  // Bewusst der Arbeitsmarkt-Radius, nicht der Wohnort: der Nachname ist selten und
  // die Domain traegt ihn, eine Kleinstadt daneben ergaebe faktisch eine Adresse.
  // Fuer Recruiter-Filter reicht der Ballungsraum.
  location: 'Raum Düsseldorf',
  // ★ Bis zum 2026-09-12 stand hier „Ich baue Systeme". Der Satz ist die
  // Kernaussage der Seite und traegt sie im Hero, im Footer und in der
  // Social-Card. „Betreiben" ist die praezisere Behauptung: es ist restlos
  // eigene Leistung, es ist genau das Wort der gesuchten Rolle, und es haelt
  // im Gespraech, wenn jemand nach einer einzelnen Code-Stelle fragt.
...
export const CURRENT_FOCUS: FocusItem[] = [
  {
    ...
    status: 'aktiv',
    description:
      'Ein Wirt trägt sieben schwere, selten gleichzeitig gebrauchte Rollen, aber nicht genug Arbeitsspeicher für alle. Ein Regler entscheidet am gemessenen freien Speicher, wer starten darf: reicht er, weicht niemand, sonst weichen zuerst die leeren Rollen, die am längsten ungenutzt sind, sauber mit Zustandssicherung. Wer eine Rolle gerade benutzt, wird nie verdrängt; reicht der Speicher trotzdem nicht, kommt eine Absage im Klartext statt eines Abschusses. Ungenutzte Rollen schalten sich nach zwanzig Minuten Leerlauf selbst ab. Dasselbe gilt inzwischen für gewöhnliche Web-Dienste: sieben selten benutzte Anwendungen starten beim ersten Aufruf und schlafen danach wieder ein. Als Nächstes will ich wissen, welche Dienste diese Behandlung noch verdienen, ohne dass jemand es merkt.',
  ...
  },
  {
    ...
    status: 'aktiv',
    description:
      'Nachschlagewerk, Karte, Adress-Suche und Routenplanung liegen vollständig im Haus. Der Adress-Index war der interessante Teil: die verbreitete Standardlösung verlangt hundert Gigabyte Datenbank und mehr Arbeitsspeicher, als der Rechner hat, also liegt hier ein eigener Index, der auf genau eine Frage zugeschnitten ist. Offen ist, die Adress-Suche direkt an die Routenplanung zu hängen.',
  ...
  },
  {
    ...
    status: 'aktiv',
    description:
      'Die Off-Site-Sicherung deckt inzwischen jeden Host ab, inklusive des Arbeitsrechners und des verschlüsselten Bündels mit den Zugangsdaten, und eine tägliche Vollständigkeits-Prüfung meldet jede Lücke von selbst. Zur vierteljährlichen Byte-Probe ist eine monatliche auf Dienst-Ebene gekommen: die Datenbank fährt aus der Sicherung in einem Wegwerf-Container hoch, und ihr Schema wird gegen die laufende Instanz gehalten. Als Nächstes will ich kontrollierte Ausfälle auslösen und zusehen, ob das System sie abfängt oder nur meldet.',
  },
  {
...
/** Sprachunabhängiger Schlüssel je Disziplin. Trägt das Icon. */
export type StackKey = 'infra' | 'backend' | 'frontend' | 'ai';
...
export const STACK: StackCategory[] = [
  {
    key: 'infra',
    label: 'Infra & Ops',
    items: ['Proxmox · LXC · KVM', 'Docker Compose', 'systemd · Units & Timer', 'nginx · Caddy · cloudflared', 'VPN', 'Paketfilter', 'better-auth · SSO', 'Backup & Restore', 'Prometheus · Grafana', 'Mailserver', 'Windows Server · Active Directory'],
  },
  {
    key: 'backend',
    label: 'Backend',
...
export const PRINCIPLES: Principle[] = [
  ...
  // wiederholen: wer selbst baut, hat auch die Reparaturpflicht.
  {
    ...
    body: 'Wer selbst baut, betreibt auch selbst, nachts und im Fehlerfall. Deshalb kommt nichts herein, das ich im Zweifel nicht auseinandernehmen kann. Das begrenzt den Umfang, und genau so ist es gemeint.',
  },
  {
...
const ABOUT_EN = {
  ...
  tagline: 'I run systems that fit together and are mine.',
  bio: 'It matters to me that I understand what runs on my own machines. A third-party product can triple its price or switch off its API tomorrow, and then you are stuck. So as little as possible runs here that I do not have in my own hands, even when that means the solutions come out smaller, and everything is documented so that somebody else could take it over.',
};
...
const CURRENT_FOCUS_EN: FocusItem[] = [
  {
    ...
    status: 'aktiv',
    description:
      'One host carries seven heavy roles that are rarely needed at the same time, but not enough memory for all of them. A controller decides by measured free memory who gets to start: if there is enough, nobody has to yield, otherwise the empty roles that have been unused longest yield first, cleanly and with their state saved. Anyone actually using a role is never evicted; if memory still falls short, the answer is a plain refusal rather than a kill. Idle roles switch themselves off after twenty minutes. The same now applies to ordinary web services: seven rarely used applications start on the first request and go back to sleep afterwards. Next I want to know which further services deserve the same treatment without anyone noticing.',
    projectId: 'ressourcen-arbiter',
  },
  {
  ...
  },
  {
    ...
    status: 'aktiv',
    description:
      'Off-site backup now covers every host, including my workstation and the encrypted bundle holding the credentials, and a daily completeness check reports any gap on its own. Alongside the quarterly byte-level drill there is now a monthly one at service level: the database boots from the backup in a throwaway container and its schema is held against the live instance. Next come controlled failure exercises, so the system absorbs disruptions on its own and does not merely report them.',
  },
  {
...
const PRINCIPLES_EN: Principle[] = [
  {
    ...
    body: 'Whoever builds it also runs it, at night and when it breaks. So nothing goes in that I could not take apart if I had to. That limits the scope, and it is meant to.',
  },
  {
