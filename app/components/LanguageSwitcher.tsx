...
import { EN_SLUGS, EN_SLUGS_REVERSE, LOCALES, isLocale, type Locale } from '@/lib/i18n/config';
import { t } from '@/lib/i18n/dict';

/**
 * DE/EN-Umschalter: verlinkt die aktuelle Seite in der jeweils anderen Sprache
 * (gleicher Pfad, getauschtes Locale-Prefix). Die proxy.ts setzt beim Navigieren
...
export function LanguageSwitcher({ locale }: { locale: Locale }) {
  ...
  function hrefFor(target: Locale): string {
    ...
    // Sichtbare EN-Slugs (projects/about/…) in beide Richtungen übersetzen,
    // sonst erzeugt der Umschalter auf /en/projects den toten Pfad /de/projects.
    const second = parts[2];
    if (second) {
      if (target === 'en' && EN_SLUGS[second]) parts[2] = EN_SLUGS[second];
      if (target === 'de' && EN_SLUGS_REVERSE[second]) parts[2] = EN_SLUGS_REVERSE[second];
    }
