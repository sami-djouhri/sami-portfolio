...
export const PROJECT_DETAILS_EN_1: Record<string, ProjectDetail> = {
  saganta: {
    ...
    approach: [
      ...
      'Everything shared lives as its own package in a pnpm monorepo that Turborepo builds along the dependency graph (UI components with a command palette (⌘K), design tokens, an auth library and an SDK), built once and consumed by every app instead of copied. That is what carries the unified look and the "one brand" feel.',
      ...
      'Identity runs through a lean auth service of my own built on better-auth rather than a foreign identity provider. An early attempt on an external OIDC stack was deliberately dropped: an own, understandable session layer fits the suite better.',
    ...
    result: [
      ...
      'Mail, news, calendar, assets and a project deck run in production; the mail part bundles external accounts instead of duplicating a mail server.',
    ...
    decisions: [
      ...
      {
        title: 'Bundle mail instead of rebuilding a mail server',
        body: 'The mail part aggregates existing external accounts and sends over their paths rather than dragging along a mail server. That buys a unified view without the operational weight of a second mail stack.',
      },
      {
        title: 'An own backend for assets and the project deck',
        body: 'Where no mature third-party backend fits, I build one: own DB, own logic, clean coupling to the other domains instead of a bolted-on tool.',
      },
      {
        title: 'Shared code as a package, not copy-paste',
        body: 'What several apps need (UI components, design tokens, an auth library) lives in versioned monorepo packages that every app consumes. The suite looks all of a piece, and one fix lands everywhere instead of being re-applied frontend by frontend.',
      },
    ],
    timeline: [
    ...
    lessons: [
      'BFFs are the right seam: they absorb the foreign complexity so the frontend never sees it.',
      'A lean auth service you build yourself can outdo a heavyweight provider when it is cut exactly to the suite, down to issuing the token for the native apps.',
      'Putting web and mobile on one identity saves the entire second login path; the mobile token just derives from the same session.',
      'A suite grows domain by domain, not in a big bang.',
    ],
    metrics: [
  ...
  homelab: {
    ...
    result: [
      ...
      'The knowledge gateway is searchable across ten adapters, and an active route probe flags public domains about to tip over before anyone notices.',
    ...
    decisions: [
      {
        title: 'Own routing instead of a service mesh',
      ...
      },
      {
        ...
        body: 'Rather than give each service its own push channel, everything flows through MQTT → brain-bus rules → ntfy/Discord. Dedup and cooldown live in one place.',
      },
    ],
    timeline: [
    ...
    metrics: [
      { label: 'Live services', value: '163 across the fleet' },
    ...
    lessons: [
      'A service map earns its keep from around 20 containers on. Below that, a notebook does the job.',
      'Hardening usually stalls on a handful of images: accept those as permanent exceptions and keep everything else clean.',
      'Drift is a standing check, not a snapshot. Diffing hourly against the canonical source catches the gap before it turns into an incident.',
      'Alerts belong centralized, not per service. A bus with dedup and cooldown heads off the alert flood that solo on-call otherwise goes numb to.',
    ],
  },
  ...
  lernen: {
    ...
    approach: [
      'Six standalone Next.js apps instead of one monolith. Each with its own SQLite database embedded in the app (better-sqlite3), its own theme, its own auth; what does not belong together does not get built together.',
    ...
    result: [
      'Six apps live on a shared core, with honest empty states everywhere instead of a mock default; an earlier monolith was retired in favor of the split apps.',
      'An exam simulator pulls real mock exams from domain-specific question pools and grades them per topic, instead of grinding through a fixed list.',
      ...
      'Native Android builds for the apps, with their own password login in the shared core rather than pure third-party sign-in.',
    ],
    decisions: [
      {
        title: 'Six apps instead of one',
    ...
    lessons: [
      'Several small apps on a shared design system beat a monolith with a module system almost every time; the dead monolith is the best proof.',
      'Inventing empty states is always wrong. An empty state with a clear statement is the right answer.',
      'An entitlement belongs modeled as a verified claim, not a set flag, or access outlives the subscription that paid for it.',
    ],
  },
  ...
  marktwatch: {
    ...
    approach: [
      ...
      'A layered false-positive defense instead of a single filter: first a free, rule-based pattern check (accessories, spare parts, damage, bundles, variant listings), then a small language model as cheap pre-triage only for borderline cases, both behind a shared gate that the real-time alert and the daily digest must equally pass.',
      'Quantitative deal math instead of a flat discount: market statistics from a time-weighted 30-day window (realized auction prices against open asks, robust dispersion via median absolute deviation), then a lognormal-t model that yields not just an expected margin but the bad case (a 5% quantile of the profit and the expected shortfall below it) plus a Kelly-style stake and a probabilistic safe-maximum bid.',
    ...
    result: [
      ...
      'A "buy" carries a traceable probability: expected margin together with the bad case (5% quantile and the expected shortfall below it), not just a percent-below-median number.',
    ...
    decisions: [
      {
        title: 'Probability instead of a point estimate',
        body: 'A single "fair price" hides the risk. A lognormal-t model with a small sample size yields the 5% quantile of the profit and the expected shortfall below it, plus a Kelly-style stake. So behind "buy" stands a downside, not just a mean.',
      },
      {
      ...
      },
      {
        ...
        body: 'A language model on every hit would be slow and expensive. The free rule filter decides the clear cases; the model is pulled only on uncertainty and against a daily budget, and fails open on an outage instead of stopping the run.',
      },
      {
    ...
    lessons: [
      'A deal is only good once its bad case is bearable too. The mean alone hides the risk; the 5% quantile and the expected shortfall below it make it visible and can be enforced as a hard gate.',
      'A false-alarm burst teaches quickly that scattered filters are not enough. Only a shared gate that both the real-time and the digest path must pass keeps signal and noise apart for good.',
      'An expensive model belongs only at the borderline cases: free rule logic decides the clear ones, the model is budgeted and fails open instead of dragging the whole run down with it.',
      'Mock-as-default flips the burden of proof: not "don\'t forget the live switch" but "arm it on purpose". That is the difference between safe and mostly safe.',
    ],
  },
  ...
  shops: {
    ...
    architecture: {
      summary:
        'A canonical hardening source on the control plane feeds every shop instance the same template bundle. Each instance runs isolated with its own data volume behind a shared reverse proxy, so new instances inherit the hardening instead of reassembling it.',
      tiers: [
        {
          label: 'Access',
    ...
    result: [
      ...
      'New instances start from the finished hardening bundle instead of a bare WordPress install.',
    ...
    decisions: [
      {
        ...
        body: 'Per-shop drift was the core problem. Keeping the templates canonical solves it: the shop reads what the control plane writes instead of maintaining its own copy.',
      },
      {
    ...
    lessons: [
      'Multi-shop hosting only scales when the hardening is canonical. Per-shop upkeep gets more expensive and less safe with every instance.',
      'A uniform deploy path is worth more than any single optimization: it keeps three shops from becoming three special cases.',
    ],
  },
  ...
  'wissens-foederation': {
    ...
    approach: [
      ...
      'Deliberate honesty about the limits: only the adapters that are really wired in get listed. When doc drift once claimed extra sources, they were checked and removed rather than shown off as a feature.',
    ],
    architecture: {
      summary:
    ...
    result: [
      'Ten real sources are searchable through a single search API, every backend access token-secured.',
      ...
      'Every search runs as an event over the own bus, so the knowledge layer is observable itself rather than a blind spot.',
    ],
    decisions: [
      {
        title: 'On-query aggregation instead of an own index',
      ...
      },
      {
        ...
        body: 'Each source gets an adapter against the same interface, and each adapter carries its own token-authenticated connection. That keeps the sources cleanly encapsulated, lets new ones dock without touching the core, and gives every backend its own bounded trust boundary instead of a shared master key.',
      },
      {
        title: 'Documented limits, not a dressed-up scope',
        body: 'Only what is really wired in gets listed. When doc drift once claimed more sources than existed, that was checked and corrected rather than left standing as a feature. An honest source list beats an impressive one that does not hold.',
      },
    ],
    metrics: [
      {
        label: 'Wired-in sources',
        value: '10, heterogeneous',
      },
      {
        label: 'Aggregation',
    ...
    timeline: [
      ...
      {
        when: '2026 H2',
        what: 'Binding to the event bus, grown to ten sources',
      },
    ],
    lessons: [
      'Federating at query time spares the most expensive class of bug: a second index that answers wrong between two syncs.',
      'A shared adapter interface turns wiring in a source into a module question instead of a core question.',
      'A source list has to be checked against reality, not against your own docs; perceived scope and actual scope drift apart otherwise.',
    ],
  },

  'home-digital-twin': {
    problem:
      'Home Assistant holds a strong data base, but the frontend it ships is an endless wall of cards: switches, sensors and histories pile up in lists, while the actual object, the house, appears nowhere. A house is thought of in space: living room, kitchen, office, floors. Control should follow that geography, not an entity table. I wanted to actually see my home rather than type it out: in 2D for quick everyday control, in 3D for the spatial overview of rooms and floors, and both on the same live status instead of two truths drifting apart. The hard part is less the rendering than the separation: layout knowledge (which room, which floor, where a device sits) belongs to the surface; device truth (on, off, temperature) belongs to home automation. Mix the two and every new device forces a frontend rebuild.',
    approach: [
      ...
      'A live bridge over WebSocket instead of polling: state changes show up the moment they happen. Smart home should feel smart: lights flip instantly, not in second-long jumps.',
    ...
    result: [
      'A spatial view of the house in 2D and 3D, both on the same live status instead of two separate truths.',
      ...
      'The access token never leaves the server: the frontend controls through the auth capsule instead of carrying credentials itself.',
    ],
    decisions: [
      {
    ...
    lessons: [
      'Spatial layout beats any card configuration: people think in rooms and floors, not entity IDs.',
      'Maintaining 2D and 3D in parallel pays off exactly when a shared data layer gives it without duplicate work. Two views on one truth cost little; two truths cost a lot.',
      'Drawing the line between layout knowledge and device truth from the start spares the rebuild later: new devices become a config question, not a code question.',
      'A token belongs encapsulated server-side, not in the browser. The auth capsule costs effort once and then lifts the security problem off every new view.',
    ],
  },
  ...
  'homelab-app': {
    ...
    approach: [
      'A native Android app with Jetpack Compose and an MVVM cut rather than a hybrid wrapper, because it is used daily and should sit inside system functions like biometrics and the camera, not just get opened now and then.',
    ...
    decisions: [
      ...
      {
        title: 'Pairing instead of a password prompt',
        body: 'Instead of a password in a form, the device itself creates a non-exportable key during pairing and exchanges it for a certificate. So there is no secret to be typed or intercepted, and access is bound to this exact device.',
      },
      {
    ...
    lessons: [
      'A mobile ops tool only pays off once access is nailed down to the device level; otherwise you build a convenient door into a private network.',
      'A device-bound key plus a certificate is, in the end, less work than handling one more password correctly, and far harder to abuse.',
      'An honest demo mode is worth gold: the app can be shown and tested in public without a single real network detail leaving the repository.',
    ],
  },

  'homelab-sentinel': {
    problem:
      'A home lab running around the clock needs an on-call person, and that is me alone. Without a push channel, that means staring at dashboards constantly and still missing the one event that matters. I wanted the opposite: a sentinel that speaks up on its own when something wobbles, that puts the decision in my hands instead of quietly making it, and that survives a restart without leaving dead buttons behind in the chat.',
    approach: [
      'Push over pull as a principle: a bot reports actively instead of me polling it, and reuses the chat channel\'s existing mobile push as a pager rather than standing up a second notification stack.',
      ...
      'The bot is a consumer of the own event bus, not a second measurement layer. Signals get deduplicated centrally, triaged with cooldown and delivered bundled, instead of every service learning its own push path.',
    ...
    architecture: {
      summary:
        'The bot consumes the event bus rather than measuring anything itself. Signals run from the central bus into the bot core, which routes them to topic-scoped cogs, raises a release button for any intervening step, and sends the result as a chat push, mobile included. Consequential steps run only after my release.',
      tiers: [
        {
          label: 'Source',
          nodes: [
    ...
    result: [
      ...
      'A release button replaces silent automation: transient things retry on their own up to a limit, and an expired release still executes on a real click instead of dead-ending.',
    ...
    decisions: [
      {
        ...
        body: 'A pure push tool would be quicker to stand up. But an own bot brings command interface, action dispatch and integrated logic under one roof, which pays off the moment the sentinel should not only report but also act on demand.',
      },
      {
        title: 'Bus as alert source, bot as consumer',
        body: 'Rather than teach every service its own bot push, everything runs first through the event bus with dedup and cooldown, and the bot is just one of several consumers. That heads off the alert flood that solo on-call otherwise goes numb to.',
      },
      {
    ...
    lessons: [
      'Push over pull is what makes solo on-call sustainable at all. Poll actively and you will eventually miss the one event that matters.',
      'Self-healing needs a release threshold. Transient things may be retried silently, consequential ones belong before my eyes; otherwise you trade an outage for an intervention at the wrong time.',
      'A button whose state lives only in memory is a lie after the next restart. Meaning belongs in the action itself; then the button outlives the bot.',
      'Before anything is shown, the bot has to know where to send it. A missing channel binding lets every alert run silently into nothing: the first thing to suspect when nothing arrives.',
    ],
  },

  'ai-vision': {
    problem:
      'First-generation edge hardware is stubborn: tight memory, an older OS and toolchain layer, a GPU architecture almost nothing off the shelf fits. Standard install paths break at spots you only see on impact. And yet the built-in GPU is usable once you understand the platform quirks and steer around them. The goal: dependable image recognition locally in my own network instead of a cloud dependency, so other own services can request image tasks without data leaving the house.',
    approach: [
    ...
    result: [
      ...
      'A documented, reproducible platform path: future models and new recognition tasks build on the same foundation instead of suffering the bring-up all over again.',
    ...
    decisions: [
      {
        ...
        body: 'A hardware upgrade is often simply not an option on the first generation. Instead of running at it, I built a documented path that accepts the platform\'s quirks and gets productive precisely through that. The stumbling blocks become recorded knowledge, not recurring pain.',
      },
      {
    ...
    lessons: [
      'Working with the platform instead of against it is the only productive path on first-generation edge hardware: an upgrade is often off the table, the documented path is not.',
      'An API in front of the model decouples the consumers from the hardware. New tasks move in without anyone binding code to the GPU.',
      'Simple-class flash does not survive ML loads. Persistence on sturdy storage is the cheapest lasting insurance against creeping data loss.',
      'Documenting the platform stumbling blocks exactly once pays off at the first hardware rework: what is written down can be brought back up instead of suffered again.',
    ],
  },
};
