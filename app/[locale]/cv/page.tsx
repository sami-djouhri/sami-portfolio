...
import { getCv, getZiel } from '@/lib/cv';
import { CV_OEFFENTLICH } from '@/lib/site';
...
export async function generateMetadata(
  ...
  return {
    ...
    description:
      locale === 'en'
        ? 'Résumé to read or print. Focused on Linux infrastructure, operations and automation, plus my own suites.'
        : 'Lebenslauf zum Lesen oder Drucken. Schwerpunkt Linux-Infrastruktur, Betrieb und Automation, dazu eigene Suiten.',
...
export default async function CvPage(props: { params: Promise<{ locale: string }> }) {
  ...
  // Vorübergehend abgeschaltet (lib/cv.ts CV_OEFFENTLICH). Bewusst `notFound()`
  // und keine Umleitung: die Seite ist nicht umgezogen, sie ist gerade nicht
  // veröffentlicht. Der Rest der Komponente bleibt unangetastet stehen, damit
  // das Zurückstellen nur den Schalter braucht.
  if (!CV_OEFFENTLICH) notFound();
  ...
  return (
    <div className="relative">
      ...
      <main id="main" className="mx-auto max-w-3xl px-6 pb-32 pt-12 sm:px-8 sm:pt-16">
        ...
        <Section command="cat praxis.log" title={locale === 'en' ? 'Hands-on practice' : 'Praxis'}>
          <p className="-mt-2 mb-8 max-w-2xl text-sm leading-relaxed text-muted">
            {locale === 'en'
              ? 'Not employment, but continuous self-directed practice: self-built and self-run, with responsibility for operation, hardening and recovery. Formal experience in a company would be the next step.'
              : 'Kein Anstellungsverhältnis, sondern durchgehende Eigenpraxis: selbst aufgebaut und selbst betrieben, mit Verantwortung für Betrieb, Härtung und Wiederanlauf. Praxis in einem Unternehmen wäre der nächste Schritt.'}
          </p>
            ...
            {cv.projectsFeatured.map((p) => {
              const card = (
                <div className="h-full rounded-lg border border-border bg-surface/60 p-4 transition-colors group-hover:border-border-strong group-hover:bg-surface-2/40">
                  ...
                  {p.id ? (
                    <p className="mt-2 font-mono text-[0.65rem] uppercase tracking-widest text-muted-dim no-print">
