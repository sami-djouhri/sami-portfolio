  ...
  return {
    ...
    description:
      locale === 'en'
        ...
        : 'Wo ich gerade stehe: was aktiv läuft, was als Nächstes ansteht, was noch in Design ist, plus eine ehrliche Momentaufnahme.',
    ...
    openGraph: {
      description:
        locale === 'en'
          ...
          : 'Eine ehrliche Momentaufnahme: was gerade aktiv läuft, was als Nächstes ansteht und was noch in Design ist.',
    },
  };
}
          ...
          <PageHeader
            ...
            lead={
              en
                ...
                : 'Eine ehrliche Momentaufnahme: manches im Bau, manches in Design, manches in der Warteschlange. Statusänderungen ziehen hier ein, sobald sie passieren.'
            }
            command
          />
        </HeaderVisual>
          ...
          <div className="prose-editorial drop-cap mt-6">
            <p>
              {en ? (
                <>
                  ...
                  {aktiv[0]?.title ?? 'my own suite'}, building it out further, domain by domain,
                  instead of bolting a rented solution onto it somewhere. {aktiv[0]?.description ?? ''}
                </>
              ) : (
                <>
                  Gerade zieht sich ein roter Faden durch fast jeden Werktag:{' '}
                  {aktiv[0]?.title ?? 'die eigene Suite'} weiter ausbauen, Domäne für Domäne, statt
                  irgendwo eine gemietete Lösung dranzuhängen. {aktiv[0]?.description ?? ''}
                </>
              )}
            </p>
            {aktiv[1] ? (
              <p>
                {en ? (
                  <>
                    Alongside it, {aktiv[1].title} keeps running, not a side project, but the kind
                    of discipline I enjoy most: {aktiv[1].description?.toLowerCase()}
                  </>
                ) : (
                  <>
                    Daneben läuft {aktiv[1].title} weiter, kein Nebenprojekt, sondern die Disziplin,
                    die mir am meisten Spaß macht: {aktiv[1].description?.toLowerCase()}
                  </>
                )}
              </p>
            ) : null}
            ...
            <p>
              {en
                ...
                : 'Abseits davon halte ich es bewusst ruhig: lieber wenige Dinge sauber zu Ende bringen als viele Bälle in der Luft. Was hier nicht steht, ist meist Absicht und nicht Vergesslichkeit.'}
            </p>
          </div>
        </section>
